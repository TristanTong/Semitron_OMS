...
(function ($) {
    $.addFlex = function (t, p) {
        ...
        var g = {
                ...
                if (p.dataType == 'json') {
                    if (data.rows != null) {
                        $.each(data.rows, function (i, row) {
                            ...
                            //start 童荣辉修改20120515 使复选框支持自动选上
                            var trClassHtml = "";
                            //在启用了checkbox的情形下判断行有设置列checked999为最后一列数据是否为true or false
                            if (p.showcheckbox == true && p.cols.indexOf("checked999") != -1 && row.cell[parseInt(row.cell.length - 1)] == "true") {
                                trClassHtml = "class='trSelected'";
                                if (i % 2 && p.striped) {
                                    trClassHtml = "class='erow trSelected'";
                                }
                                tbhtml.push(trClassHtml);
                            }
                            else {
                                if (i % 2 && p.striped) {
                                    tbhtml.push(" class='erow'");
                            ...
                            $(ths).each(function (j) {
                                ...
                                if (idx == "-1" || idx == "-2") { //checkbox
                                    ...
                                    idx == "-1" && div.push("<input type='checkbox' id='chk_", row.id, "' class='itemchk' " + checkHtml + "  value='", row.id, "'/>");
                                    //end 童荣辉
            ...
            populate: function () { //get latest data 
                ...
                for (var cindex = 0, clength = p.colModel.length; cindex < clength; cindex++) {
                    colNames += "," + p.colModel[cindex].name;
                }
                ...
                /*Start 童荣辉 20121101 使控件支持Session判断*/
                $.ajax({
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data != null && data.error != null) {
                                    if (p.onError) {
                                        p.onError(data); g.hideLoading();
                                    }
                                } else {
                                    g.addData(data);
                                }
                            },
                            error: function (data) { try { if (p.onError) { p.onError(data); } else { } g.hideLoading(); } catch (e) { } }
                        });

                        return true;
                    }
                });
                /*End 童荣辉 20121101 使控件支持Session判断*/
            },
            doSearch: function () {
            ...
            getSelectedNotRows: function () {
                var items = [];
                if (!p.rowbinddata) {
                    alert("请将属性rowbinddata设置为true");
                }
                $("tr[class!='trSelected']", g.bDiv).each(function () {
                    items.push($(this).attr("CH").split('_FG$SP_'));
                });
                return items;
            },
            getAllRows: function () {
                ...
                return items;
            }, //getAllRows add by Tristan 20120517 获取所有加载到当页的数据行
            getTotal: function () {
                return p.total;
            },
                            ...
                            $("input.itemchk", this).each(function () {
                                $(this).click(function (e) {
                                    ...
                                    if (p.onrowchecked) {
                                        ...
                                        //有子项取消选中同时取消表格头中全选状态。
                                        $(".hDivBox .cth .noborder").get(0).checked = false;
                                    }
                                    e.stopPropagation();
                                });
                            });
        ...
        g.bDiv.setAttribute("id", "gridBodyDiv");
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<div class="pGroup"><div class="pFirst pButton" title="转到第一页"><span></span></div><div class="pPrev pButton" title="转到上一页"><span></span></div> </div><div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">当前 <input type="text" title="按回车键进行跳转" size="1" value="1" /> ,总页数 <span> 1 </span></span></div><div class="btnseparator"></div><div class="pGroup"> <div class="pNext pButton" title="转到下一页"><span></span></div><div class="pLast pButton" title="转到最后一页"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="刷新"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div>';
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'>快速检索：<input type='text' size='30' name='q' class='qsbox' title='按回车键进行查询'/> <select name='qtype'>" + sopt + "</select> <input type='button' class='btnHigh' name='qclearbtn' value='清空' /></div>");
    ...
    //重新定向高度(increaseHeight:高度增量)
    $.fn.flexReHeight = function (increaseHeight) {
        var grdHeight = document.documentElement.clientHeight - increaseHeight;
        var h = grdHeight;
        var p = { height: h };
        return this.each(function () {
            if (this.grid) {
                $.extend(this.p, p);
                document.getElementById("gridBodyDiv").style.height = h + 'px';
            }

        });
    };
    //重新定向高度(increaseHeight:高度增量)
    //多个flexTable情况，指定tableId
    $.fn.flexReHeight = function (increaseHeight, tableId) {
        var grdHeight = document.documentElement.clientHeight - increaseHeight;
        var h = grdHeight;
        var p = { height: h };
        return this.each(function () {
            if (this.grid) {
                $.extend(this.p, p);
                $("div[id='gridBodyDiv']").each(function (i) {
                    if ($(this).find("table[id='" + tableId + "']") && $(this).find("table[id='" + tableId + "']").length > 0) {
    ...
    //zuo
    $.fn.getSelectedNotRows = function () {
        if (this[0].grid) {
            return this[0].grid.getSelectedNotRows();
