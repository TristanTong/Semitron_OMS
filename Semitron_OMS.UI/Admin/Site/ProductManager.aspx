<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ProductManager.aspx.cs"
    Inherits="Semitron_OMS.UI.Admin.Site.ProductManager" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>网站后台系统 - 展示产品管理</title>
    <%--CSS Ref--%>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#FlexigridTable").flexigrid({
                ...
                url: '/Handle/Site/ProductShowHandle.ashx', //数据请求地址
                dataType: 'json', //请求数据的格式
                extParam: [{ name: "meth", value: "GetProductShow" }, { name: "TimeType", value: $("#sltTimeType").val() }, { name: "startTime", value: $("#txtBeginTime").val() }, { name: "AvailFlag", value: $("#sltAvailFlag").val()}], //扩展参数
                colModel: [//表格的题头与索要填充的内容。
                        ...
                        { display: '产品名称', name: 'Name', width: 200, sortable: true, align: 'left' },
                        { display: '分类', name: 'TypeName', width: 100, sortable: true, align: 'center' },
	    				...
	    				{ display: '首页图片路径', name: 'IcoPath', width: 280, sortable: true, align: 'left' },
                        { display: 'HTML文件名', name: 'HtmlPath', width: 140, sortable: true, align: 'left' },
            ...
            $("#btnEdit").click(function () {
                var obj = GetSelectedRow("FlexigridTable", "展示产品");
                if (!obj) {
                    return false;
                }
                if (obj[0][2] == "无效") {
                    artDialog.tips("当前展示产品记录已无效，无法完成此操作。");
                    return false;
                }
                var id = obj[0][1];

                GetProductShowById(id)
                AddOrEdit(id, "Edit");
                return false;
            });
            //删除
            $("#btnDelete").click(function () {
                ...
                if (obj[0][2] == "无效") {
                    artDialog.tips("当前展示产品已无效，请勿重复操作。");
                    return false;
                }
                var id = obj[0][1];
                DelProductShow(id);
    ...
    <script type="text/javascript">
        ...
        function Search() {
            ...
            var p = { extParam: [   //扩展参数
                        {name: "meth", value: "GetProductShow" },
        ...
        //根据Id获得展示产品记录
        function GetProductShowById(id) {
            var url = "/Handle/Site/ProductShowHandle.ashx";
            var data = { "meth": "GetProductShowById", "Id": id };
            var successFun = function (json) {
                if (json.State == "0") {
                    ...
                    $("#txtIcoPathE").val(json.IcoPath);
                    $("#sltLangE").val(json.Lang);
                    $("#sltTypeIdE").val(json.TypeId);
        ...
        function AddOrEdit(id, type) {
            ...
            $.dialog({
                ...
                ok: function () {
                    ...
                    var IcoPath = Trim($("#txtIcoPathE").val());
                    var Lang = Trim($("#sltLangE").val());
                    var TypeId = Trim($("#sltTypeIdE").val());
                    ...
                    var url = "/Handle/Site/ProductShowHandle.ashx";
                    //新增展示产品
                    var data = { "meth": "AddProductShow", "Code": Code, "Name": Name, "SK": SK, "IsShowInMain": IsShowInMain, "HtmlPath": HtmlPath, "PageHeight": PageHeight, "IcoPath": IcoPath, "Lang": Lang, "TypeId": TypeId };
                    //编辑展示产品
                    if (id && id != "") {
                        data = { "meth": "EditProductShow", "Code": Code, "Name": Name, "SK": SK, "IsShowInMain": IsShowInMain, "HtmlPath": HtmlPath, "PageHeight": PageHeight, "IcoPath": IcoPath, "Lang": Lang, "TypeId": TypeId, "Id": id };
                    }
                    var errorFun = function (x, e) {
                        alert(x.responseText);
                    };
        ...
        //删除展示产品
        function DelProductShow(id) {
            art.dialog.confirm('展示产品删除后将无法恢复，你确认要删除吗？', function () {
                //调用删除展示产品方法
                var url = "/Handle/Site/ProductShowHandle.ashx";
                var data = { "meth": "DelProductShow", "Id": id };
                var successFun = function (json) {
                    if (json.State == "0") {
                        artDialog.alert(json.Info);
                        return false;
    ...
    <script type="text/javascript">
        ...
        KindEditor.ready(function (K) {
            editor = K.create('#content1', {
                ...
                uploadJson: '/Handle/Site/UploadHandle.ashx?Type=product',
                fileManagerJson: '/Handle/Site/FileManagerHandle.ashx?Type=product',
    ...
    <div>
        <div class="SearchDiv">
            <table style="width: 100%" id="tableSearch">
                ...
                <tr>
                    <td style="text-align: right;">
                        <div id="divTypeId" style="float: left;">
                            <div class="spandiv">
                                分类：</div>
                            <select id="sltTypeId" class="txt " runat="server" style="width: 125px">
                                <option value="">--请选择--</option>
                            </select>
                        </div>
        ...
        <div id="divEdit">
            <table style="width: 100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr class="trMO ">
                    ...
                    <td class="tdLeft tdRemarkWidth">
                        <select class="txt" id="sltTypeIdE" runat="server" style="width: 85px">
                ...
                <tr class="trMO ">
                    ...
                    <td class="tdLeft tdRemarkWidth" colspan="3">
                        <input type="text" id="txtIcoPathE" class="txt" maxlength="128" style="width: 250px" />
                        <label style="color: Red">
                            温馨提示：请右键图片属性复制。</label>
                    </td>
                    <td>
                    </td>
                </tr>
