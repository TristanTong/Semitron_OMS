<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CustomerManager.aspx.cs"
    Inherits="Semitron_OMS.UI.Admin.CRM.CustomerManager" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>订单管理系统 - 客户管理</title>
    <%--CSS Ref--%>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#FlexigridTable").flexigrid({
                ...
                url: '/Handle/CRM/CustomerHandle.ashx', //数据请求地址
                dataType: 'json', //请求数据的格式
                extParam: [{ name: "meth", value: "GetCustomer" }, { name: "TimeType", value: $("#sltTimeType").val() }, { name: "startTime", value: $("#txtBeginTime").val() }, { name: "AvailFlag", value: $("#sltAvailFlag").val() }], //扩展参数
            ...
            $("#btnEdit").click(function () {
                var obj = GetSelectedRow("FlexigridTable", "客户");
                if (!obj) {
                    return false;
                }
                if (obj[0][2] == "无效") {
                    artDialog.tips("当前客户记录已无效，无法完成此操作。");
                    return false;
                }
                var id = obj[0][1];
            ...
            //关联销售
            $("#btnBindSalesMan").click(function () {
                var obj = GetSelectedRow("FlexigridTable", "客户");
                if (!obj) {
                    return false;
                }
                if (obj[0][2] == "无效") {
                    artDialog.tips("当前客户已无效，无法完成此操作。");
                    return false;
                }
                var id = obj[0][1];
                $("#span1").text(obj[0][4]);
                $("#span2").text(obj[0][3]);
                LoadAdminBindCustomerTable(id);
                BindOperations(id);
                return false;
            });
        });
    ...
    <script type="text/javascript">
        ...
        function GetCustomerById(id) {
            ...
            var data = { "meth": "GetCustomerById", "Id": id };
            var successFun = function (json) {
                if (json.State == "0") {
                    artDialog.alert(json.Info);
                    return false;
                } else {
                    $("#txtCCodeE").val(json.CCode);
                    $("#txtCustomerNameE").val(json.CustomerName);
                    $("#txtSKE").val(json.SK);
        ...
        function AddOrEdit(id, type) {
            var dialogTitle = "客户管理>>新增客户";
            if (type == "Edit") {
                dialogTitle = "客户管理>>编辑客户"
            }
            ...
            $.dialog({
                ...
                init: function () {
                    if (type == "Add") {
                        var url = "/Handle/CRM/CustomerHandle.ashx";
                        var data = { "meth": "GenerateCCodeAndPath" };
                        var errorFun = function (x, e) {
                            alert(x.responseText);
                        };
                        var successFun = function (json) {
                            if (!json || json.State == 0) {
                                ...
                                $("#txtCCodeE").attr("disabled", "disabled");
                                $("#divEdit .txt").each(function () {
                                    $(this).val("");
                                });
                                $("#txtCCodeE").val(json.Info);
                                $("#txtareaCustomerAddressE").val("");
                                return true;
                            }
                        };
                        JsAjax(url, data, successFun, errorFun);
                ...
                ok: function () {
                    var CCode = Trim($("#txtCCodeE").val());
                    var CustomerName = Trim($("#txtCustomerNameE").val());
        ...
        function DelCustomer(id) {
            art.dialog.confirm('客户删除后将无法恢复，你确认要删除吗？', function () {
                //调用删除客户方法
                var url = "/Handle/CRM/CustomerHandle.ashx";
                var data = { "meth": "DelCustomer", "Id": id };
                var successFun = function (json) {
                    if (json.State == "0") {
                        artDialog.alert(json.Info);
                        return false;
    ...
    <script type="text/javascript">
        //加载销售人员表格
        function LoadAdminBindCustomerTable(CustomerId) {
            $("#AdminBindCustomerTable").flexigrid({
                width: 580, //表格宽度
                height: 250, //表格高度
                url: '/Handle/OMS/AdminBindCustomerHandle.ashx', //数据请求地址
                dataType: 'json', //请求数据的格式
                extParam: [{ name: "meth", value: "GetAdminBindCustomerList" }, { name: "CustomerId", value: CustomerId }], //扩展参数
                colModel: [//表格的题头与索要填充的内容。           
                { display: '序号', name: 'rownum', toggle: false, iskey: true, sortable: true, width: 60, align: 'center', hide: true },
                        ...
                        { display: '销售人员ID', name: 'AdminID', width: 60, sortable: true, align: 'center' },
                        { display: '销售人员', name: 'Name', width: 120, sortable: true, align: 'left' },
        ...
        //刷新列表
        function RefreshAdminBindCustomerTable(CustomerId) {
            var p = {
                extParam: [{ name: "meth", value: "GetAdminBindCustomerList" }, { name: "CustomerId", value: CustomerId }] //扩展参数 
            };
            p.newp = 1;         //跳转到第一页。
            $("#AdminBindCustomerTable").flexOptions(p).flexReload();
        }

        //关联销售人员
        function BindOperations(CustomerId) {
            $.dialog({
                content: $("#divAdminBindCustomer").get(0),
                id: 'divAdminBindCustomer',
                title: "关联销售人员",
                init: function () {
                },
                ok: function () {
                    art.dialog.confirm("是否确认关联操作？", function () {
                        var obj = $("#AdminBindCustomerTable").getSelectedRows();  //获取当前页被勾选的行
                        var objAll = $("#AdminBindCustomerTable").getAllRows();  //获取当前页的所有的行
                        var IdList = "";
                        var IdListAll = "";
                        if (obj.length < 0) {
                            artDialog.tips("获取销售人员行数据异常。");
                            return false;
                        }
                        ...
                        var url = "/Handle/OMS/AdminBindCustomerHandle.ashx";
                        ...
                        var successFun = function (json) {
                            ...
                            artDialog.confirm(json.Info + "是否继续关联其它销售人员？",
                             function () {
                                 RefreshAdminBindCustomerTable(CustomerId);    //刷新表格
                             },
                            function () {
                                artDialog.tips("退出关联操作。");
                                art.dialog.list["divAdminBindCustomer"].close();
                                $("#AdminBindCustomerTable").flexReload();
            ...
            RefreshAdminBindCustomerTable(CustomerId);
        }
    </script>
</head>
<body>
    <form id="form1" runat="server">
        <div>
                ...
                <table style="width: 100%" id="tableSearch">
                    <tr>
                        <td style="text-align: right;">
                            <div id="divCCode" style="float: left;">
                                <div class="spandiv">
                                    编码：
                                </div>
                                <input style="width: 120px;" id="txtCCode" class="txt  " runat="server" maxlength="16" />m
                            </div>
                            <div id="divCustomerName" style="float: left;">
                                <div class="spandiv">
                                    客户名称：
                                </div>
                                <input style="width: 120px;" id="txtCustomerName" class="txt  " runat="server" maxlength="128" />m
                            </div>
            ...
            <div style="height: 34px" id="divOperation">
                <div class="OperationDiv" style="float: left;">
                    ...
                    <asp:Button runat="server" Text="关联销售" CssClass="btnHigh" ID="btnBindSalesMan" />
                </div>
        ...
        <div class="dialogDiv">
                ...
                <table style="width: 100%" border="0" cellspacing="0" cellpadding="0">
                    <tr class="trMO ">
                        ...
                        <td class="tdLeft tdRemarkWidth ">
                            <input type="text" id="txtCCodeE" class="txt   txtdis" maxlength="16" />
                        </td>
                        <td class="tdRight tdParamDWidth">名称：
                        </td>
                        <td class="tdLeft tdRemarkWidth">
                            <input type="text" id="txtCustomerNameE" class="txt" maxlength="128" />
                        </td>
                        <td></td>
                    </tr>
                    <tr class="trMO ">
            ...
            <%--关联销售--%>
            <div id="divAdminBindCustomer">
                <table>
                    <tr>
                        <td>
                            <span style="font-weight: bold;">客户编码:</span><span id="span1"></span>&nbsp;&nbsp;&nbsp;&nbsp;
                        <span style="font-weight: bold;">客户名称:</span><span id="span2"></span>&nbsp;&nbsp;&nbsp;&nbsp;
                        </td>
                    </tr>
                </table>
                <table id="AdminBindCustomerTable" style="display: none;">
                </table>
            </div>
        </div>
    </form>
