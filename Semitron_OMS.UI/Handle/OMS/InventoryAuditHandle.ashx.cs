...
{
    /// <summary>
    /// InventoryAuditHandle 的摘要说明
    /// </summary>
    public class InventoryAuditHandle : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {

        log4net.ILog _myLogger = log4net.LogManager.GetLogger(typeof(InventoryAuditHandle));
        Semitron_OMS.BLL.OMS.InventoryAuditBLL _bllInventoryAudit = new Semitron_OMS.BLL.OMS.InventoryAuditBLL();
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(methStr))
            {
                ...
                {
                    //获取列表
                    case "GetInventoryAudit":
                        context.Response.Write(GetInventoryAudit());
                        break;
                }
            }
        }

        private string GetInventoryAudit()
        {
            //查询条件信息
            ...
            try
            {
                DataSet ds = this._bllInventoryAudit.GetInventoryPageData(searchInfo, out o_RowsCount);
            ...
            {
                dt = new DataTable();
                _myLogger.Error("登陆用户名：" + _adminModel.Username + "客户机IP:" + _request.UserHostAddress + "，获取库存变更信息出现异常:" + ex.Message, ex);
            }
            ...
            if (strType == "ExportExcel")
            {
                ...
                try
                {
                    ...
                    bool bA = bllOL.AddExecute("InventoryAuditHandle", filename, "", (int)OperationsType.Export);
                    if (bA)
                    {
                        result.State = 1;
