...
{
    /// <summary>
    /// SupplierHandle 的摘要说明
    /// </summary>
    public class SupplierHandle : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        //供应商对象
        log4net.ILog _myLogger = log4net.LogManager.GetLogger(typeof(SupplierHandle));
        Semitron_OMS.BLL.CRM.SupplierBLL _bllSupplier = new Semitron_OMS.BLL.CRM.SupplierBLL();
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(methStr))
            {
                switch (methStr)
                {
                    //获取供应商列表
                    case "GetSupplier":
                        context.Response.Write(GetSupplier());
                        break;
                    //新增供应商
                    case "AddSupplier":
                        context.Response.Write(AddSupplier());
                        break;
                    //修改供应商
                    case "EditSupplier":
                        context.Response.Write(EditSupplier());
                        break;
                    //删除供应商
                    case "DelSupplier":
                        context.Response.Write(DelSupplier());
                        break;
                    //获取供应商详细信息
                    case "GetSupplierById":
                        context.Response.Write(GetSupplierById());
                        break;
                    //生成编码
                    case "GenerateSCodeAndPath":
                        context.Response.Write(GenerateSCodeAndPath());
                        break;
                }
            }
        ...
        private string GetSupplier()
        {
            ...
            SQLOperateHelper.AddSQLFilter(lstFilter, SQLOperateHelper.GetSQLFilter("SupplierName", _request.Form["SupplierName"], ConditionEnm.AllLike));
            SQLOperateHelper.AddSQLFilter(lstFilter, SQLOperateHelper.GetSQLFilter("SCode", _request.Form["SCode"], ConditionEnm.AllLike));
            ...
            try
            {
                DataSet ds = _bllSupplier.GetSupplierPageData(searchInfo, out o_RowsCount);
            ...
            {
                dt = new DataTable();
                _myLogger.Error("登陆用户名：" + _adminModel.Username + "客户机IP:" + _request.UserHostAddress + "，获取供应商信息出现异常:" + ex.Message, ex);
            }
        ...
        /// <returns></returns>
        private string GetNewModel(SupplierModel model)
        {
            PageResult result = new PageResult();
            if (_request.Form["SCode"] == null
                || _request.Form["SupplierName"] == null
                || _request.Form["SupplierAddress"] == null
            ...
            SQLOperateHelper.SetEntityFiledValue(model, "SCode", _request.Form["SCode"]);
            SQLOperateHelper.SetEntityFiledValue(model, "SupplierName", _request.Form["SupplierName"]);
            SQLOperateHelper.SetEntityFiledValue(model, "SupplierAddress", _request.Form["SupplierAddress"]);
        ...
        private string AddSupplier()
        {
            ...
            try
            {
                ...
                model.SCode = JsonConvert.DeserializeObject<PageResult>(this.GenerateSCodeAndPath()).Info;
                string strResult = this._bllSupplier.ValidateAndAdd(model);
            ...
            {
                result.State = 0;
                result.Info = "新增供应商出现异常，请联系管理员。";
                _myLogger.Error("登陆用户名：" + _adminModel.Username + ",客户机IP:" +
                    _request.UserHostAddress + "，新增供应商出现异常:" + ex.Message, ex);
            }
            return result.ToString();
        }
        ...
        private string EditSupplier()
        {
            ...
            try
            {
                SupplierModel model = this._bllSupplier.GetModel(iId);
                string strGetResult = this.GetNewModel(model);
                if (strGetResult != "OK")
                {
                    result.State = 0;
                ...
                string strResult = this._bllSupplier.ValidateAndUpdate(model);
                if (strResult == "OK")
                {
                    result.State = 1;
            ...
            {
                result.State = 0;
                result.Info = "编辑供应商出现异常，请联系管理员！";
                _myLogger.Error("登陆用户名：" + _adminModel.Username + "，客户机IP:" + HttpContext.Current.Request.UserHostAddress + "，编辑供应商出现异常：" + ex.Message, ex);
            }
            return result.ToString();
        }
        ...
        private string DelSupplier()
        {
            ...
            try
            {
                string strResult = this._bllSupplier.ValidateAndDelSupplier(iId);
                if (strResult == "OK")
                {
                    result.State = 1;
                    result.Info = "删除供应商成功。";
            ...
            {
                result.State = 0;
                result.Info = "删除供应商出现异常！";
                _myLogger.Error("登陆用户名：" + _adminModel.Username + "，客户机IP:" + HttpContext.Current.Request.UserHostAddress + "，删除供应商出现异常：" + ex.Message, ex);
            }
            return result.ToString();
        }
        ...
        private string GetSupplierById()
        {
            ...
            {
                result.State = 0;
                result.Info = "获取供应商详细信息出现异常！";
                _myLogger.Error("登陆用户名：" + _adminModel.Username + "，客户机IP:" + HttpContext.Current.Request.UserHostAddress + "，获取供应商详细信息出现异常：" + ex.Message, ex);
            }
            return result.ToString();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private string GenerateSCodeAndPath()
        {
            PageResult result = new PageResult();
            result.Info = "无法生成编码！";
            ...
            try
            {
                int iMax = this._bllSupplier.GetMaxId();
                SupplierModel model = new SupplierModel();
                //首次生成
                if (iMax == 1)
                {
                ...
                {
                    model = this._bllSupplier.GetModel(iMax - 1);
                }
                if (model != null && !string.IsNullOrEmpty(model.SCode))
                {
                    result.Info = "S" + (int.Parse(model.SCode.Remove(0, 1)) + 1).ToString().PadLeft(4, '0');
                    result.State = 1;
