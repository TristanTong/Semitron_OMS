...
namespace Semitron_OMS.DAL.OMS
{
    ...
    public partial class InventoryDAL
    {
        ...
        {
            //查询表名
            string strTableName = " dbo.Inventory AS G WITH (NOLOCK) LEFT JOIN Warehouse AS W ON G.WCodeBelong=W.WCode LEFT JOIN ProductInfo AS P WITH (NOLOCK) ON P.ProductCode=G.ProductCode";
            //查询字段
            string strGetFields = " G.ID,G.ProductCode,P.MPN,G.WCodeBelong,W.WName,G.OnHandQty,G.POQty,G.UnInQty,G.UnOutQty,UpdateTime=Convert(varchar(20),G.UpdateTime,120),G.UpdateUser ";
            //查询条件
