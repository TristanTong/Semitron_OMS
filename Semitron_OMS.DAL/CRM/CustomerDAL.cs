    ...
    public partial class CustomerDAL
    {
        ...
        public int Add(Semitron_OMS.Model.CRM.CustomerModel model)
        {
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@CustomerName", SqlDbType.NVarChar,128),
        ...
        public Semitron_OMS.Model.CRM.CustomerModel GetModel(int ID)
        {
            ...
            Semitron_OMS.Model.CRM.CustomerModel model = new Semitron_OMS.Model.CRM.CustomerModel();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Semitron_OMS.Model.CRM.CustomerModel DataRowToModel(DataRow row)
        {
            Semitron_OMS.Model.CRM.CustomerModel model = new Semitron_OMS.Model.CRM.CustomerModel();
            if (row != null)
            {
        ...
        public DataSet GetCustomerPageData(PageSearchInfo searchInfo, out int o_RowsCount)
        {
            ...
            //查询字段
            string strGetFields = " ID , CCode ,CustomerName ,ContactPerson,Tel, SK,AvailFlag = CASE AvailFlag WHEN 1 THEN '有效' ELSE '无效' END , CreateUser , CreateTime = CONVERT(VARCHAR(20), CreateTime, 120) , UpdateUser , UpdateTime = CONVERT(VARCHAR(20), UpdateTime, 120) ";
            //查询条件
        ...
        public bool ExistsByCode(string strCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from Customer");
            strSql.Append(" where CCode=@Code");
            SqlParameter[] parameters = {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Customer set AvailFlag=" + iStatus);
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
