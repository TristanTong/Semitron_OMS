...
{
    /// <summary>
    /// 数据访问类:ProductShowDAL
    /// </summary>
    public partial class ProductShowDAL
    {
        public ProductShowDAL()
        { }
        #region  BasicMethod
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "ProductShow");
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into ProductShow(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ProductShow set ");
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from ProductShow ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        public Semitron_OMS.Model.Site.ProductShowModel GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,Code,Name,Lang,TypeId,SK,IsShowInMain,IcoPath,HtmlPath,PageHeight,AvailFlag,CreateUser,CreateTime,UpdateUser,UpdateTime from ProductShow ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
            ...
            Semitron_OMS.Model.Site.ProductShowModel model = new Semitron_OMS.Model.Site.ProductShowModel();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Semitron_OMS.Model.Site.ProductShowModel DataRowToModel(DataRow row)
        {
            Semitron_OMS.Model.Site.ProductShowModel model = new Semitron_OMS.Model.Site.ProductShowModel();
            if (row != null)
            {
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM ProductShow ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from ProductShow T ");
        ...
        public DataSet GetList(int PageSize,int PageIndex,string strWhere)
        {
            ...
            parameters[0].Value = "ProductShow";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Name , IcoPath , HtmlPath , PageHeight ");
            strSql.Append(" FROM ProductShow WHERE  AvailFlag = 1 " + (bInMain ? " AND IsShowInMain = 1" : "") + (string.IsNullOrEmpty(strLang) ? (" AND lang='en'") : (" AND lang='" + strLang + "'")) + (string.IsNullOrEmpty(strType) ? "" : (" AND CAST(TypeId AS VARCHAR(50))='" + strType + "'")) + " ORDER BY SK ASC,CreateTime Desc");
            return SQLNotifier.GetDataTable(strSql.ToString(), strSql.ToString(), ConstantValue.TableNames.ProductShow, null);
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT Id , Pid , Name , Height , BID , Lang ");
            strSql.Append("FROM ( SELECT    Id = CAST(ID AS VARCHAR(32)) , Pid , Name = TypeName , Height = 0 , BID = '' , SK , Lang FROM      ProductType WHERE   AvailFlag = 1 AND  Lang = '" + strLang + "' UNION ALL SELECT TOP 30 Id = CAST(SUBSTRING(HtmlPath, 1, LEN(HtmlPath) - 4) AS VARCHAR(50)) , Pid = TypeId , Name , Height = PageHeight , BID = '' , SK , Lang FROM      dbo.ProductShow WHERE     AvailFlag = 1 AND Lang = '" + strLang + "' AND SUBSTRING(HtmlPath, 0, LEN(HtmlPath) - 4) IS NOT NULL ) AS A  WHERE 1 = 1 " + (string.IsNullOrEmpty(strLang) ? (" AND lang='en'") : (" AND lang='" + strLang + "'")) + " ORDER BY SK ASC ,Name ASC");
        ...
        public DataSet GetProductShowPageData(Semitron_OMS.Common.PageSearchInfo searchInfo, out int o_RowsCount)
        {
            //查询表名
            string strTableName = " ProductShow ";
            //查询字段
            string strGetFields = " ID , Code , Name , SK,Lang ,TypeName=(SELECT TOP 1 TypeName FROM dbo.ProductType WHERE TypeId=ID ), IsShowInMain = CASE IsShowInMain WHEN 1 THEN '是' ELSE '否' END , IcoPath,HtmlPath , PageHeight , AvailFlag = CASE AvailFlag WHEN 1 THEN '有效' ELSE '无效' END , CreateUser , CreateTime = CONVERT(VARCHAR(20), CreateTime, 120) , UpdateUser , UpdateTime = CONVERT(VARCHAR(20), UpdateTime, 120) ";
            //查询条件
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ProductShow set AvailFlag=" + iStatus);
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
