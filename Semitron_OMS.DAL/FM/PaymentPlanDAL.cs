...
namespace Semitron_OMS.DAL.FM
{
    ...
    public partial class PaymentPlanDAL
    {
        ...
        public int Add(Semitron_OMS.Model.FM.PaymentPlanModel model)
        {
            ...
            strSql.Append("CorporationID,PONO,POPlanID,PaymentPlanDate,ProductCode,MPN,SupplierID,Qty,BuyStandardCurrency,BuyPrice,BuyCost,BuyRealCurrency,BuyExchangeRate,BuyRealPrice,BuyRealTotal,VendorPaymentTypeID,IsSupplierVATInvoice,SupplierVATInvoice,IsPaySupplier,OtherFee,OtherFeeRemark,State,CreateTime,CreateUser,UpdateTime,UpdateUser)");
            ...
            parameters[25].Value = model.UpdateUser;
        ...
        public Semitron_OMS.Model.FM.PaymentPlanModel GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,CorporationID,PONO,POPlanID,PaymentPlanDate,ProductCode,MPN,SupplierID,Qty,BuyStandardCurrency,BuyPrice,BuyCost,BuyRealCurrency,BuyExchangeRate,BuyRealPrice,BuyRealTotal,VendorPaymentTypeID,IsSupplierVATInvoice,SupplierVATInvoice,IsPaySupplier,OtherFee,OtherFeeRemark,State,CreateTime,CreateUser,UpdateTime,UpdateUser from PaymentPlan ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        {
            //查询表名
            string strTableName = "  dbo.PaymentPlan AS P WITH(NOLOCK) LEFT JOIN dbo.Supplier AS S WITH(NOLOCK) ON S.ID=P.SupplierID LEFT JOIN dbo.PO AS O ON O.PONO=P.PONO AND O.State!=-100 LEFT JOIN dbo.Corporation AS C ON C.ID=P.CorporationID LEFT JOIN dbo.PaymentType AS T ON T.ID=P.VendorPaymentTypeID LEFT JOIN dbo.CurrencyType AS CT ON CT.ID=P.BuyRealCurrency LEFT JOIN dbo.CurrencyType AS CT1 ON CT1.ID=P.BuyStandardCurrency LEFT JOIN (SELECT  ObjId ,FileNum = COUNT(1) FROM dbo.Attachment WITH ( NOLOCK ) WHERE ObjType = 'PaymentPlanFilePath' AND AvailFlag = 1 GROUP BY ObjId) AS A ON A.ObjId=P.ID LEFT JOIN dbo.GodownEntryDetail AS D WITH(NOLOCK) ON D.POPlanId=P.POPlanID AND D.AvailFlag=1 LEFT JOIN dbo.GodownEntry AS E WITH(NOLOCK) ON E.ID=D.GodownEntryID AND E.State=1 ";
            //查询字段
            string strGetFields = " P.ID,State=CASE WHEN P.State=1 THEN '有效' ELSE '无效' END ,PaymentPlanDate=CONVERT(VARCHAR(10),P.PaymentPlanDate,120),CorporationID=C.CompanyName,P.PONO,P.ProductCode,P.MPN,S.SupplierName,IsPaySupplier=CASE WHEN P.IsPaySupplier=1 THEN '是' ELSE '否' END ,IsSupplierVATInvoice=CASE WHEN P.IsSupplierVATInvoice=1 THEN '是' ELSE '否' END ,P.Qty,VendorPaymentTypeID=T.PaymentType,P.BuyExchangeRate,BuyRealCurrency=CT.CurrencyName,P.BuyRealPrice,P.BuyRealTotal,BuyStandardCurrency=CT1.CurrencyName,P.BuyPrice,P.BuyCost,P.OtherFee,P.CreateUser,CreateTime = CONVERT(VARCHAR(20), P.CreateTime, 120) , P.UpdateUser , UpdateTime = CONVERT(VARCHAR(20), P.UpdateTime, 120),FileNum=ISNULL(A.FileNum,0),O.InnerBuyer,E.EntryNo,IsInStock=CASE WHEN E.IsApproved=1 THEN '是' ELSE '否' END , InStockDate= CONVERT(VARCHAR(10), E.InStockDate, 120) ";
            //查询条件
