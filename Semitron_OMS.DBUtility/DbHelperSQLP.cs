...
namespace Semitron_OMS.DBUtility
{
    ...
    public class DbHelperSQLP
    {
        ...
        public SqlTransaction tran;//设置一个全局的事务对象
        ...
        /// <param name="tableName">DataSet结果中的表名</param>
        /// <returns>DataSet</returns>
        public DataSet RunProcedure(string storedProcName, IDataParameter[] parameters, string tableName, out int rowsCount, int index)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet dataSet = new DataSet();
                ...
                sqlDA.Fill(dataSet, tableName);
                if (string.IsNullOrEmpty(sqlDA.SelectCommand.Parameters[index].Value.ToString()))
                {
                    rowsCount = 0;
                }
                else
                {
                    rowsCount = Convert.ToInt32(sqlDA.SelectCommand.Parameters[index].Value);
                }
                connection.Close();
                return dataSet;
            }
